...
contract NftAuction is Initializable{
    ...
    // 状态变量
    mapping (uint256 => Auctcion) public auctions;
    //下一个拍卖ID
    ...
    function creatAuction(uint256  _duration,uint256 _startPrice,address _nftAddress, uint256 _tokenId)  public{
        ...
        auctions[nextAuctionId]=Auctcion({
        seller:msg.sender,
        duration:_duration,
    ...
    //     Auctcion storage auction=auctions[_auctionID];
    //     //判断当前拍卖是否结束
    //         require(!auction.ended && auction.startTime + auction.duration < block.timestamp,"Auction has ended");
    //         //判断出价是否大于当前最高价 
    //         require(msg.varvalue>auction.highestBid && msg.value >= auction.startPrice,"Bid must be greater than the highest bid")
    //         //退回之前的最高出价者
    ...
    //判断当前拍卖是否结束
        require(!auction.ended && auction.startTime + auction.duration < block.timestamp,"Auction has ended");
        //判断出价是否大于当前最高价 
        require(msg.value>auction.highestBid && msg.value >= auction.startPrice,"Bid must be greater than the highest bid");
        //退回之前的最高出价者
        if(auction.highestBidder!=address(0)){
            payable(auction.highestBidder).transfer(auction.highestBid);
        }
